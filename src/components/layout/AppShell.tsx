...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const isAuthPage = pathname === '/login' || pathname === '/signup' || pathname === '/forgot-password' || pathname?.startsWith('/p/');
          ...
          <div className="container flex h-20 items-center justify-between max-w-screen-2xl px-6">
            ...
            <div className="flex items-center gap-3">
               {user ? (
                 <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <button className="flex items-center gap-2 p-1.5 rounded-2xl hover:bg-slate-100 dark:hover:bg-slate-800 transition-all outline-none group">
                          <div className="text-right hidden sm:block px-2">
                              ...
                              <p className="text-xs font-black text-slate-800 dark:text-white truncate max-w-[100px]">{user?.displayName || 'المعلم'}</p>
                          </div>
                          <div className="relative">
                    ...
                    <DropdownMenuContent align="end" className="w-72 rounded-[1.5rem] p-3 shadow-2xl border-slate-100 dark:border-slate-800 animate-in zoom-in-95 duration-200">
                      ...
                      <div className="p-1 space-y-1">
                          ...
                          <DropdownMenuItem onClick={handleSignOut} className="flex items-center justify-between w-full p-3 rounded-xl hover:bg-rose-50 dark:hover:bg-rose-950/20 group cursor-pointer text-rose-600 transition-colors">
                              <span className="font-bold text-sm">تسجيل الخروج</span>
                              <div className="p-2 bg-rose-50 dark:bg-rose-900/20 text-rose-500 rounded-lg group-hover:bg-rose-600 group-hover:text-white transition-colors">
                 ...
                 <div className="flex items-center gap-2">
                    <Button asChild variant="ghost" className="rounded-xl font-black text-sm hover:bg-primary/5 h-11 px-5 transition-all">
                    ...
                    <Button asChild className="rounded-xl font-black text-sm bg-primary shadow-lg shadow-primary/20 hover:scale-105 transition-all h-11 px-6">
